    ...
    describe("implicit") {
        describe("instance") {
            ...
            it("returns given instance") {
                ...
                expect { try maker.makeInstance(resolver: DummyResolver3()) } == 42
            }
        }
        describe("provider") {
            it("produces maker with correct type signature") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = provider { passedResolver3 = $0 }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
        ...
        describe("factory") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = factory { (r, _: Void) in passedResolver3 = r }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("calls builder with given argument") {
                ...
                _ = try? maker.makeInstance(arg: 42, resolver: DummyResolver3())
            ...
            describe("multiple arguments") {
                it("works with 2 arguments") {
                    ...
                    let arguments = (1, 1.0)
                    expect { try maker.makeInstance(arg: arguments, resolver: DummyResolver3()) } == 42
                }
        ...
        describe("singleton") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = singleton { passedResolver3 = $0 }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("rethrows error from builder") {
        ...
        describe("multiton") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = multiton { (r, _: Void) in passedResolver3 = r }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("calls builder with given argument") {
    ...
    describe("contexted") {
        ...
        describe("provider") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = environment.provider { r, _ in passedResolver3 = r }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("calls builder with given context") {
                ...
                _ = try? maker.makeInstance(context: 42, resolver: DummyResolver3())
        ...
        describe("factory") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = environment.factory { (r, _, _: Void) in passedResolver3 = r }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("calls builder with given context") {
    ...
    describe("scoped") {
        ...
        describe("singleton") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = environment.singleton { passedResolver3 = $0 }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("calls builder with given context") {
                ...
                _ = try? maker.makeInstance(context: "context", resolver: DummyResolver3())
        ...
        describe("multiton") {
            ...
            it("calls builder with given resolver") {
                var passedResolver3: Resolver3?
                let resolver = DummyResolver3()
                let maker = environment.multiton { (r, _, _: Void) in passedResolver3 = r }
                _ = try? maker.makeInstance(resolver: resolver)
                expect(passedResolver3) === resolver
            }
            it("calls builder with given context") {
